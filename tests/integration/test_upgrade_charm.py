...
async def test_upgrade_edge_with_local_in_isolation(ops_test: OpsTest, charm_under_test):
    ...
    assert ops_test.model
    sh.juju.deploy(app_name, model=ops_test.model.name, channel="1/edge", trust=True)
...
async def test_upgrade_local_with_local_with_relations(ops_test: OpsTest, charm_under_test):
    ...
    assert ops_test.model
    sh.juju.deploy(
        "prometheus-k8s", "prom", model=ops_test.model.name, channel="1/edge", trust=True
    )
    sh.juju.deploy("karma-k8s", "karma", model=ops_test.model.name, channel="1/edge", trust=True)
