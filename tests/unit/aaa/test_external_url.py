...
async def test_build_and_deploy(ops_test: OpsTest, charm_under_test):
    ...
    await ops_test.model.wait_for_idle(status="active", timeout=deploy_timeout)  # type: ignore[union-attr]  # noqa: E501
...
async def test_units_can_communicate_to_form_a_cluster(ops_test: OpsTest):
    await ops_test.model.applications[app_name].scale(scale=3)  # type: ignore[union-attr]
...
async def test_workload_is_locally_reachable_with_external_url_with_path(ops_test: OpsTest):
    ...
    await ops_test.model.wait_for_idle(status="active", timeout=config_timeout)  # type: ignore[union-attr]  # noqa: E501
